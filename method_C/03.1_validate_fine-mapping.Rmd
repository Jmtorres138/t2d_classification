...
alt.df <- fread(analysis.dir  %&% "classified-loci_alternative.txt")
...
alt.df$assigned_20 <- gsub("other","unclassified",alt.df$assigned_20)
...
shapiro.test(grp1$islet.max.vs.null) # p-value = 7.016e-06
shapiro.test(grp1$multi.max.vs.null) # p-value = 2.037e-12
...
shapiro.test(grp2$islet.max.vs.null) # p-value = 3.919e-06
shapiro.test(grp2$multi.max.vs.null) # p-value = 9.251e-11
# Data is not likely normally distributed 
...
comp.df <- build_comp_df(alt.df)

grp1 <- filter(comp.df,group=="islet")
wilcox.test(grp1$islet.max.vs.null,grp1$multi.max.vs.null) 


plt.list <- make_test_plots(alt.df)
  
ggsave(plot=plt.list[[1]],filename=plot.dir%&%"Wilcox-compare-max_alternative.png",height=3,width=7)
ggsave(plot=plt.list[[2]],filename=plot.dir%&%"Wilcox-compare-num_alternative.png",height=3,width=7)
