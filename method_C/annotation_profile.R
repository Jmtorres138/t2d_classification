...
divvy_ppa_snp <- function(loc.id,snp.id,mode="full",weights=TRUE){
  ...
  if (mode=="full"){
    score.vec <- c(coding.vec,strongenh.vec,weakenh.vec,genenh.vec,prom.vec,genetrans.vec)
  } else{
  ...
  names(ppa.vec) <- c("coding_islet","coding_muscle","coding_adipose","coding_liver",
                      ...
                      "prom_islet","prom_muscle","prom_adipose","prom_liver",
                      "genetrans_islet","genetrans_muscle","genetrans_adipose","genetrans_liver")
...
divvy_ppa_loc <- function(loc.id,mode="full",weights=TRUE,scaled=FALSE){
  ...
  out.df2 <- cbind(dplyr::select(out.df2,one_of("Locus.ID","SNPID")),dplyr::select(out.df2,coding_islet:genetrans_liver))
