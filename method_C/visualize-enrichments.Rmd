...
eqtl_enrich_plot <- function(enrich.df,myname){
  ggplot(data=enrich.df,aes(x=tissue,y=enrichment)) + 
    ...
    ggtitle(myname) +
    theme(axis.text.x=element_text(angle=30,vjust=0.6)) #+ coord_cartesian(ylim=c(0,20)) + +
...
ggsave(plot=plt4,filename = plot.dir %&% "downsampled_enrichment-eqtl-specific_weighted.png",width=9,height=6.5)
