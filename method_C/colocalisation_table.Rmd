...
s1 <- filter(candw.df,snpcount<=10,maxppa>=0.5) %>% arrange(.,desc(maxppa))
s2 <- filter(candu.df,snpcount<=10,maxppa>=0.5) %>% arrange(.,desc(maxppa))


grob3 <- make_table_plot_filtered(s1)
ggsave(plot=grob3,filename=plot.dir %&%
         "filtered-table_weighted_all.png",height=20,width=20)
grob3b <- make_table_plot_filtered(s2)
ggsave(plot=grob3b,filename=plot.dir %&%
         "filtered-table_unweighted_all.png",height=20,width=20)
...
s <- filter(coloc.df,Locus=="70_4",gene_name=="SETD9",Tissue=="Islet") %>% arrange(.,desc(CLPP))
id <- "chr5:56238811"
filter(cred.df,CondID=="70_4",SNPID==id)$PPA # PPA

"chr5:56196604"
"chr5:56206529"

s2 <- filter(coloc.df,Locus=="70_4",gene_name=="SETD9") %>% arrange(.,desc(CLPP))
View(s2)

id2 <- "chr5:56205643"
filter(cred.df,CondID=="70_4",SNPID==id2)$PPA

#chr5:56196604

s3 <- filter(coloc.df,Locus=="70_4",SNP_ID=="5_56196604")%>% arrange(.,desc(CLPP))
