...
write.table(x=df.1,file=work.dir %&% "analysis_files/classified-loci_unweighted_renamed.txt",
            sep="\t",row.names=F,col.names=T)
df.2 <- fread(work.dir %&% "analysis_files/" %&% 
                     "classified-loci_weighted.txt") %>% rename_locid(.)
write.table(x=df.2,file=work.dir %&% "analysis_files/classified-loci_weighted_renamed.txt",
            sep="\t",row.names=F,col.names=T)
...
enrich_test_v2 <- function(phys.vec, tissue, class.df,iter=10000){
  if (tissue == "peripheral"){
    full.vec <- select(class.df,one_of("liver","muscle","adipose")) %>% rowSums(.)
    query.vec <- filter(class.df,Cond.ID%in%phys.vec) %>% select(.,one_of("liver","muscle","adipose")) %>% rowSums(.)
  } else{
    full.vec <- select(class.df,one_of(tissue)) %>% as.matrix(.) %>% as.numeric(.)
    query.vec <- filter(class.df,Cond.ID%in%phys.vec) %>% select(.,one_of(tissue)) %>% as.matrix(.) %>% as.numeric(.)
  }
...
write.table(x=df3,file = work.dir %&% "analysis_files/" %&% 
              "phys-enrich-V2_unweighted.txt",sep="\t",quote=F,row.names=F)

df4 <- build_enrich_df_v2(weighted.df)
write.table(x=df4,file = work.dir %&% "analysis_files/" %&% 
