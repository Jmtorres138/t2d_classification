...
(filter(full.df,other<0.10) %>% dim(.))[1] # 237 loci with other scores < 0.50
(filter(cse.df,other<0.10) %>% dim(.))[1] # 67 loci with other scores < 0.50
...
Conclusion: The number of SNPs in the credible set doesn't influence the difference in ppa scores between full and reduced methods, altough slight correlation with number of snps and value of the "other" score
...
pca_plot_annotated <- function(data.df,pca.out,pcX,pcY){
  ...
  ggplot(data=pca.df,aes_string(x=pcX,y=pcY)) + 
    ...
    scale_fill_manual(values=c("gold1","olivedrab2","brown","red","grey","black")) + 
    theme_classic() + 
    geom_label_repel(data=other.df,aes(x=PC1,y=PC2,label=refseq),
                     size=1.5,fill="grey",alpha=0.8) + 
    geom_label_repel(data=islet.df,aes(x=PC1,y=PC2,label=refseq),
                     size=1.5,fill="olivedrab2",alpha=0.8) +   
    geom_label_repel(data=adipose.df,aes(x=PC1,y=PC2,label=refseq),
                     size=1.5,fill="gold1",alpha=0.8) + 
    geom_label_repel(data=liver.df,aes(x=PC1,y=PC2,label=refseq),
                     size=1.5,fill="brown",alpha=0.8) +
    geom_label_repel(data=muscle.df,aes(x=PC1,y=PC2,label=refseq),
...
complete_pca(filter(full.df,other<=0.10),centerScale=FALSE)
