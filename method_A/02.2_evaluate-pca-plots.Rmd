...
pca.list <- make_pca_inputs(wfull.df,centerScale=FALSE) 
pca.list.cent <- make_pca_inputs(wfull.df,centerScale=TRUE) 
...
#scree_cumsum_plot(pca.list.cent[[3]])

#loading_plot(pca.list.cent[[2]])

#pca_plot(pca.list.cent[[1]],
#                   pca.list.cent[[2]],"PC1","PC2")

plt.uncent <- complete_pca(wfull.df,centerScale=FALSE)
plt.cent <- complete_pca(wfull.df,centerScale=TRUE)
...
pca.list.pruned <- make_pca_inputs(filter(wfull.df,other<=0.1),
                                 centerScale=FALSE)  
