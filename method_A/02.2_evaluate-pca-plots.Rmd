...
scale.df <- fread(analysis.dir %&% "tissue_ppa_divvy-full-weighted-scaled.txt") %>% 
  filter(.,Locus.ID %in% keep.vec)
unscale.df <- fread(analysis.dir %&% "tissue_ppa_divvy-full-weighted-unscaled.txt") %>% 
  filter(.,Locus.ID %in% keep.vec)
...
pca.list <- make_pca_inputs(scale.df,centerScale=FALSE) 
pca.list.un <- make_pca_inputs(unscale.df,centerScale=FALSE) 
...
plt <- complete_pca(scale.df,centerScale=FALSE)
plt.un <- complete_pca(unscale.df,centerScale=FALSE)
...
pca.list.scale.pruned <- make_pca_inputs(filter(scale.df,other<=0.1),
                                 centerScale=FALSE)  
complete_pca(filter(scale.df,other<=0.1),centerScale=FALSE)


pca.list.unscale.pruned <- make_pca_inputs(filter(unscale.df,other<=0.1),
                                 centerScale=FALSE)  
complete_pca(filter(unscale.df,other<=0.1),centerScale=FALSE)
...
pruneA.plt <- pca_plot_annotated(pca.list.scale.pruned[[1]],
                   pca.list.scale.pruned[[2]],"PC1","PC2")
pruneB.plt <- pca_plot_annotated(pca.list.scale.pruned[[1]],
                   pca.list.scale.pruned[[2]],"PC2","PC3")
ggsave(plot=pruneA.plt,filename=plot.dir%&%"pca_pruned-full-weighted-LocScaled-pc1-pc2.pdf",
       height=6,width=6)
ggsave(plot=pruneB.plt,filename=plot.dir%&%"pca_pruned-full-weighted-LocScaled-pc2-pc3.pdf",
       height=6,width=6)
...
pruneA.plt.un <- pca_plot_annotated(pca.list.unscale.pruned[[1]],
                   pca.list.scale.pruned[[2]],"PC1","PC2")
pruneB.plt.un <- pca_plot_annotated(pca.list.unscale.pruned[[1]],
                   pca.list.scale.pruned[[2]],"PC2","PC3")
ggsave(plot=pruneA.plt.un,filename=plot.dir%&%"pca_pruned-full-weighted-pc1-pc2.pdf",
       height=6,width=6)
ggsave(plot=pruneB.plt.un,filename=plot.dir%&%"pca_pruned-full-weighted-pc2-pc3.pdf",
       height=6,width=6)
