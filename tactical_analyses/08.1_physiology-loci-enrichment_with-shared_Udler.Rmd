...
df4 <- build_enrich_df(weighted.df,phys.df)
write.table(x=df4,file = write.dir %&% "analysis_files/" %&% 
              "phys-enrich_weighted_with-shared_UDLER.txt",
            sep="\t",quote=F,row.names=F)

filter(df4,p<0.05,threshold!="assigned_00") %>% arrange(.,p)
...
df4b <- build_enrich_df_v2(weighted.df,phys.df)

filter(df4b,p<0.05) %>% arrange(.,p) #%>% View(.)

write.table(x=df4b,file = write.dir %&% "analysis_files/" %&% 
              "phys-enrich_weighted-V2_UDLER.txt",sep="\t",quote=F,row.names=F)
