...
annot.names <- c("Active_enhancer_1","Active_enhancer_2","Weak_enhancer","repressed.polycomb",
                 "repressed.polycomb.weak","Active_TSS","Weak_TSS", "Flanking_TSS",
                 "Strong_transcription","Weak_transcription",
                 "Bivalent_poised_TSS","Genic_enhancer","low.signal","Coding")
annot.list <- list(Active_enhancer_1,Active_enhancer_2,Weak_enhancer,repressed.polycomb,
                   repressed.polycomb.weak,Active_TSS,Weak_TSS, Flanking_TSS,
                   Strong_transcription,Weak_transcription,Bivalent_poised_TSS,
                   Genic_enhancer,low.signal,Coding)
...
get_weight_full <- function(all.df,annot.group){
  ...
  w.df <- sub %>% group_by(.,tissue) 
  weight.vec <- map(1:dim(w.df)[1],function(i){
    ifelse(sign(w.df[i,]$CI_lo)==sign(w.df[i,]$CI_hi),w.df[i,]$estimate,0)
  }) %>% as.numeric(.)
...
build_weight_full_df <- function(all.df,annot.names,annot.list){
  ...
  for (i in 1:length(annot.names)){
    ...
    build.df <- data.frame("tissue"=names(weight.vec),"annotation"=annot,
                           "weight"=2^(unname(weight.vec)),stringsAsFactors = FALSE)
...
exclude.vec <- c("low.signal","repressed.polycomb","repressed.polycomb.weak",
                 "Bivalent_poised_TSS")
...
plt <- ggplot(data=filt.df,aes(x=annotation,y=weight)) + 
...
an_plot <- function(mydf,mytitl="",mylow=-10,myhigh=10,interval=1){
  ...
  plt <- ggplot(data=mydf,aes(x=parameter,y=estimate)) +
    ...
    ylab("Log2 Fold Enrichment") + xlab("Annotation") +
...
ex.vec <- c("utr_5","distance_tss_0_5000","18_Quiescent_low_signal",
            "exon","14_Bivalent_poised_TSS","intron",
            "17_Weak_repressed_polycomb","promoter",
