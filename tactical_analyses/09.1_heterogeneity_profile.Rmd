...
sub.df$Assign.differ %>% sum(.) / length(sub.df$Assign.differ) # 41/60 (68.3%) 
sub.df$Ranks.differ %>% sum(.) / length(sub.df$Ranks.differ) # 59/60 (98.3%) with differing rank orders 

```

There are 84 loci with allelic heterogeneity 
60 have enough signals classified at the 0.2 threshold to enable comparison (i.e. two or more)
Of these 60, 41 (68.3%) have different tissue classifications (heterogeneous assignments) 
Moreover, 59 (98.3%) have different rank orders (tissue profiles) 
...
Plot all 60 loci that can be analyzed for heterogeneity (see above) 
...
ggsave(file = plot.dir %&% "tiss_het_60.png", arrangeGrob(grobs = plot.list, ncol = 7),
       width=20,height=20) 
ggsave(file = plot.dir %&% "tiss_het_60.pdf", arrangeGrob(grobs = plot.list, ncol = 7),
       width=20,height=20) 

filter(df,loc %in% loc.vec) %>% dim(.) # 176 signals at these 60 loci 
...
df60 <- sub.df
df60$same.across <- map(df60$Locus,function(l){
  vec <- filter(df,loc==l)$assigned_20 %>% unique(.) 
  vec <- vec[vec!="unclassified"]
  length(vec) == 1
}) %>% as.logical(.)
df60$same.NotShared <- map(df60$Locus,function(l){
  vec <- filter(df,loc==l)$assigned_20 %>% unique(.) 
  vec <- vec[vec!="unclassified"]  
...
loc.vec <- filter(df60,same.across==TRUE)$Locus %>% unique(.) %>% 
...
ggsave(file = plot.dir %&% "tiss_het_20_same.png", arrangeGrob(grobs = plot.list, ncol = 8),
       width=18,height=8) 
ggsave(file = plot.dir %&% "tiss_het_20_same.pdf", arrangeGrob(grobs = plot.list, ncol = 8),
       width=18,height=8) 

filter(df,loc %in% loc.vec) %>% dim(.) # 44 signals at these 20 loci 
...
loc.vec <- filter(df60,same.NotShared==TRUE)$Locus %>% unique(.) %>% 
  as.integer(.) %>% sort(.) %>% as.character(.)
plot.list <- list()
for (i in 1:length(loc.vec)){
  l <- loc.vec[i]
...
ggsave(file = plot.dir %&% "tiss_het_9_sameNoShared.png", 
       arrangeGrob(grobs = plot.list, ncol = 9),
       width=21,height=3) 
ggsave(file = plot.dir %&% "tiss_het_9_sameNoShared.pdf",
       arrangeGrob(grobs = plot.list, ncol = 9),
       width=21,height=3) 

filter(df,loc %in% loc.vec) %>% dim(.) # 20 signals at these 9 loci 

df9 <- filter(df,loc%in%loc.vec)
write.table(x=df9,file=analysis.dir%&%"het_9.txt",sep="\t",quote=F,row.names=F)
...
df60$diff.across <- map(df60$Locus,function(l){
  vec <- filter(df,loc==l)$assigned_20 %>% unique(.) 
  vec <- vec[vec!="unclassified"]
  length(vec) > 1
}) %>% as.logical(.)
df60$diff.NotShared <- map(df60$Locus,function(l){
  vec <- filter(df,loc==l)$assigned_20 %>% unique(.)
  vec <- vec[vec!="unclassified"]
  vec <- vec[vec!="shared"]
...
loc.vec <- filter(df60,diff.across==TRUE)$Locus %>% unique(.) %>% 
  as.integer(.) %>% sort(.) %>% as.character(.)
plot.list <- list()
for (i in 1:length(loc.vec)){
  l <- loc.vec[i]
...
ggsave(file = plot.dir %&% "tiss_het_40_diff.png", arrangeGrob(grobs = plot.list, ncol = 8),
       width=20,height=16) 
ggsave(file = plot.dir %&% "tiss_het_40_diff.pdf", arrangeGrob(grobs = plot.list, ncol = 8),
       width=20,height=16) 

filter(df,loc %in% loc.vec) %>% dim(.) # 132 signals at these 40 loci 
...
loc.vec <- filter(df60,diff.NotShared==TRUE)$Locus %>% unique(.) %>% 
  as.integer(.) %>% sort(.) %>% as.character(.)
plot.list <- list()
for (i in 1:length(loc.vec)){
  l <- loc.vec[i]
...
ggsave(file = plot.dir %&% "tiss_het_10_diffNotShared.png", 
       arrangeGrob(grobs = plot.list, ncol = 10),
       width=21,height=3) 
ggsave(file = plot.dir %&% "tiss_het_10_diffNotShared.pdf", 
       arrangeGrob(grobs = plot.list, ncol = 10),
       width=21,height=3) 

filter(df,loc %in% loc.vec) %>% dim(.) # 43 signals at these 10 loci 
...
same.loci <- filter(df60,same.NotShared==TRUE)$Locus %>% unique(.)
diff.loci <- filter(df60,diff.NotShared==TRUE)$Locus %>% unique(.)
