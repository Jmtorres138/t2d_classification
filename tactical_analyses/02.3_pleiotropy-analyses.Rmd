...
(dplyr::select(prim.df ,-contains("assigned_")) %>% arrange(.,desc(islet)) %>% 
   filter(.,islet > 0.9))$Locus.ID %>% length(.) # 12
(dplyr::select(prim.df ,-contains("assigned_")) %>% arrange(.,desc(islet)) %>% 
   filter(.,islet > 0.9)) %>% View(.)
# Liver
(dplyr::select(prim.df ,-contains("assigned_")) %>% arrange(.,desc(liver)) %>%
    filter(.,liver > 0.9))$Locus.ID %>% length(.) # 4
(dplyr::select(prim.df ,-contains("assigned_")) %>% arrange(.,desc(liver)) %>%
    filter(.,liver > 0.9)) %>% View(.)
# Adipose 
(dplyr::select(prim.df ,-contains("assigned_")) %>% arrange(.,desc(adipose)) %>% 
    filter(.,adipose > 0.9))$Locus.ID %>% length(.) # 1 
(dplyr::select(prim.df ,-contains("assigned_")) %>% arrange(.,desc(adipose)) %>% 
    filter(.,adipose > 0.9)) %>% View(.)
