...
build_annotation_count_df <- function(tiss.df){
  ...
  annot.list <- list(strong.enhancers,weak.enhancers,strong.promoter,
                     weak.promoter,flank.promoter,strong.gene.transcription,
                     weak.gene.transcription,genic.enhancer)
...
build_dist_df <- function(tiss.df){
  se.gr <- filter(tiss.df,V4 %in% strong.enhancers) %>% convert_to_gr(.) 
  we.gr <- filter(tiss.df,V4 %in% weak.enhancers) %>% convert_to_gr(.) 
  sp.gr <- filter(tiss.df,V4 %in% strong.promoter) %>% convert_to_gr(.) 
  wp.gr <- filter(tiss.df,V4 %in% weak.promoter) %>% convert_to_gr(.) 
  fp.gr <-  filter(tiss.df,V4 %in% flank.promoter) %>% convert_to_gr(.) 
  st.gr <- filter(tiss.df,V4 %in% strong.gene.transcription) %>% convert_to_gr(.)
  wt.gr <- filter(tiss.df,V4 %in% weak.gene.transcription) %>% convert_to_gr(.)
  ge.gr <- filter(tiss.df,V4 %in% genic.enhancer) %>% convert_to_gr(.) 
...
median_tile_plot <- function(dist.df,omit.vec=c("Genic Enhancer","Flank Promoter")){
...
mean_tile_plot <- function(dist.df,omit.vec=c("Genic Enhancer","Flank Promoter")){
  p <- ggplot(filter(dist.df,!(ref.annot%in%omit.vec),!(query.annot%in%omit.vec)),
               aes(ref.annot, query.annot)) +
