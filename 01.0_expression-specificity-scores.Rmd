...
ess_rn_scores <- function(rn.df){
  rn.df$islet <- rn.df$islet + 2
  rn.df$muscle <- rn.df$muscle + 2 
  rn.df$adipose <- rn.df$adipose + 2 
  rn.df$liver <- rn.df$liver + 2
  out.df <- c()
  ...
  for (i in 1:dim(rn.df)[1]){
    ...
    islet.score <- rn.df$islet[i] / tot
    muscle.score <- rn.df$muscle[i] / tot
    adipose.score <- rn.df$adipose[i] / tot
    liver.score <- rn.df$liver[i] / tot
