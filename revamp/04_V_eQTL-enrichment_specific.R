...
islet.eqtls <- isl.spec#sample(x=isl.spec,replace=FALSE,size=min.val)
mus.eqtls <- mus.spec#sample(x=mus.spec,replace=FALSE,size=min.val)
liv.eqtls <- liv.spec#sample(x=liv.spec,replace=FALSE,size=min.val)
adi.eqtls <- adi.spec#sample(x=adi.spec,replace=FALSE,size=min.val)
...
write.table(x=thresh2.df,file=proj.dir%&%"revamp/analysis_files/eqtl-specific-validation_weighted.txt",
            sep="\t",quote=FALSE,row.names=FALSE)
