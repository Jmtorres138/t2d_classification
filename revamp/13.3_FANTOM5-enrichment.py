...
annot_dir = "/well/mccarthy/users/jason/datasets/FANTOM5/"
...
def run_enrichments(file_list,out_file,proc_dir,shared=False):
	dir_list =  [d for d in os.listdir(annot_dir) if "enhancers" in d]
	afile_list = []
	for d in dir_list:
		bed_list = [annot_dir+d+"/"+f for f in os.listdir(annot_dir+d) if ".bed" in f]
		afile_list = afile_list + bed_list
	aname_list = [s.split("/")[-1].split(".bed")[0] for s in afile_list]
	fout = open(out_file,'w')
	fout.write("\t".join(["tissue_toa","tissue_FANTOM5","observed.count","null.mean","enrich.factor","p.val"])+"\n")
	for f in file_list:
