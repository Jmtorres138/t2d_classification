...
eval.df <- plt.list1[[3]]
sub <- filter(eval.df,threshold==0.5,group=="islet")
a <- sub$islet.max.vs.null %>% mean(.)
b <- sub$multi.max.vs.null %>% mean(.)
c <- a - b
...
write.table(x=plt.df,file=work.dir%&%"analysis_files/finemap-enrich_with-shared.txt",sep="\t",quote=F,row.names=F)
