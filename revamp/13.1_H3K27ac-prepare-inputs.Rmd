...
out.dir1 <- file.dir %&% "enrich_H3K27ac/"
...
generate_all_files <- function(){
  ...
  for (tiss in tiss.vec){
    for (t in t.vec){
      print("Tissue: " %&% tiss %&% "; Threshold: " %&% strsplit(t,split="_")[[1]][2])
      cname <- (t %>% stringToQuoser(.))
      sig.vec <- dplyr::filter(toa.df,!!cname==tiss)$Locus.ID
      save.name1 <- out.dir1 %&% "thresh" %&% strsplit(t,split="_")[[1]][2] %&% "_" %&% tiss %&% ".bed" 
      save.name2 <- out.dir2 %&% "thresh" %&% strsplit(t,split="_")[[1]][2] %&% "_" %&% tiss %&% ".ldhub.txt"      
