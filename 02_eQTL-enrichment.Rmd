...
evaluate_threshold <- function(x,collapse.df,iter){
  # x is evaluated threshold
  classified <- map(collapse.df$Locus.ID,function(id){
    sub <- filter(collapse.df,Locus.ID==id) %>% dplyr::select(-one_of("Locus.ID")) %>% 
  ...
  islet.loci <- collapse.df$Locus.ID[classified=="islet"]
  liver.loci <- collapse.df$Locus.ID[classified=="liver"]
  adipose.loci <- collapse.df$Locus.ID[classified=="adipose"]
  muscle.loci <- collapse.df$Locus.ID[classified=="muscle"]
...
evaluate_thresholds <- function(x.vec=seq(0,1,0.05),collapse.df,iter){
  out.df <- c()
  for (x in x.vec){
    print(x)
    build.df <- evaluate_threshold(x,collapse.df,iter)
...
collapse.df <- collapse_ppa_df(ppa.df)

thresh.df <- evaluate_thresholds(x.vec=seq(0,1,0.05),collapse.df,iter=100)
